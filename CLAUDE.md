...
- [ ] Dashboard (§6 step 8) → Goals → Notifications.
...
**Not yet done, and named here so it is not forgotten:** spec §4 asks for an **idempotency key on every POST that creates a money record** (`/transactions`, `/loans`). Nothing implements it, so a retried request creates a second entry. It is the next commit, before the dashboard — a dashboard built on double-counted money would look correct and be wrong. `lib/http.ts` sends no such header today, so the key will be honoured when present rather than required, which keeps the frozen contract intact.
