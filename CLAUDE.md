...
`npm run dev` proxies `/api` to the backend on :8085. **The fake API is opt-in** — `npm run dev:mock` — because a default that quietly serves fixtures is a trap: writes appear to succeed and no total moves. A production build never contains the worker: `import.meta.env.DEV` is statically false, so the dynamic import is dropped.
...
- [x] **Runnable in a browser** — `npm run dev:mock` starts MSW's browser worker against the *same* `handlers.ts` the tests use, so what a browser shows is what the tests assert. `import.meta.env.DEV` gates it, so the dynamic import is dropped from a production build entirely. It was the default until the backend existed; it is now opt-in, because fixtures that never move look exactly like a broken page.
...
| ~~Once every endpoint is live~~ **Done** | The fake API is now opt-in (`npm run dev:mock`); `npm run dev` and `dev:lan` talk to the backend. **`handlers.ts` is not deleted** — it is the test double behind all 674 frontend tests, and the browser worker is still the fastest way to work on a page with no database running. |
